...
import { PaymentState, Refund, RefundState, InitiateOrderRequest, Payment } from "jewl-core";
import { Delete, Get, Route, Security, Post, Body, Request, Put } from "tsoa";
...
export class OrderController {
    ...
    public async initiateOrder(@Request() req: WithAuthentication, @Body() body: IInitiateOrderRequest): Promise<void> {
        ...
        // TODO: Generate payment and tell stripe to process the payment
    ...
    public async cancelPendingOrders(@Request() req: WithAuthentication): Promise<void> {
        const payment = await Payment.findOne({ userId: req.user.userId, state: PaymentState.completed }).sort({ _id: -1 });
    ...
    @Get("/renew")
    public async getAutoRenewStatus(@Request() req: WithAuthentication): Promise<{ enabled: boolean }> { // TODO: <-
